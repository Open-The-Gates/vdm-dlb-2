...
import { ZoomIn, ZoomOut, Download, Maximize, ChevronLeft, ChevronRight, Loader2, AlertCircle } from "lucide-react"
    ...
    <div className="flex h-full flex-col bg-gray-200 dark:bg-gray-700">
      <div className="flex flex-shrink-0 items-center justify-between border-b bg-gray-100 p-2 dark:bg-gray-800">
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" onClick={previousPage} disabled={pageNumber <= 1}>
            <ChevronLeft className="h-4 w-4" />
          </Button>
          <span className="text-sm">
            Page {pageNumber} of {numPages || "--"}
          </span>
          <Button variant="ghost" size="icon" onClick={nextPage} disabled={!numPages || pageNumber >= numPages}>
            <ChevronRight className="h-4 w-4" />
          </Button>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" onClick={() => handleZoom(zoom - 0.2)}>
